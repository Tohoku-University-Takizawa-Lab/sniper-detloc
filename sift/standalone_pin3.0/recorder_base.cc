...
{
   // std::cout << "Crashing in sendInstruction (" << thread_data[threadid].icount << ")?" << std::endl;
   ...
   // std::cout << thread_data[threadid].num_dyn_addresses << "," << num_addresses << std::endl;
   ...
   for (BBL bbl = bbl_head; BBL_Valid(bbl); bbl = BBL_Next(bbl))
   {
      ...
      else if (current_mode == Sift::ModeDetailed)
      {
         for(INS ins = BBL_InsHead(bbl); ; ins = INS_Next(ins))
         {
            ...
            bool is_branch = INS_IsBranch(ins) && INS_HasFallThrough(ins) && !INS_IsXend(ins);
      ...
      else if (current_mode == Sift::ModeMemory)
      {
         ...
         for(INS ins = BBL_InsHead(bbl); ; ins = INS_Next(ins))
         {
            ...
            if (INS_IsBranch(ins) && INS_HasFallThrough(ins) && !INS_IsXend(ins))
            {
